...
export default defineConfig({
  ...
  dev: {
    ...(isDevServer ? {} : {assetPrefix: '/bundles/objectmerger/studio/build/' + buildId}),
    client: {
      host: 'localhost',
      port: 3033,
      protocol: 'ws'
  ...
  output: {
    manifest: true,
    assetPrefix: '/bundles/objectmerger/studio/build/' + buildId,
    distPath: {
      root: buildPath
    },
  },
  tools: {
    bundlerChain: (chain, { env }) => {
      chain.output.uniqueName('pimcore_objectmerger_bundle');
    },
  },
  plugins: [
    ...
    pluginModuleFederation({
      name: 'pimcore_objectmerger_bundle',
